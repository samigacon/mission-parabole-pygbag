...
def draw_screen():
    ...
    text_general_colored = font.render(f"f(x) = ", True, WHITE)
    a_colored = font.render(f"{A:.3f}", True, ORANGE)
    b_colored = font.render(f"{B:.1f}", True, GREEN)
    c_colored = font.render(f"{C:.2f}", True, PINK)

    screen.blit(text_general, (ecran_rect.x + 40, ecran_rect.y + 60))
    screen.blit(text_values, (ecran_rect.x + 40, ecran_rect.y + 90))
