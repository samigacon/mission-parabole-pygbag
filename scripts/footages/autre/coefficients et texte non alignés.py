...
ecran_image = pygame.image.load("ecran.png")
...
def draw_buttons():
    ...
    # Définir une police plus petite pour les libellés des boutons
    small_font = pygame.font.Font(None, 15) 
    
    # Texte "Coefficients"
    coefficients_text = font.render("Coefficients", True, WHITE)
    screen.blit(coefficients_text, (button_a_rect.left - 20, button_a_rect.top - 50))
...
reset_button_rect = reset_button_image.get_rect(topleft=(ecran_rect.right + 8, ecran_rect.top + 50))
simulation_button_rect = simulation_button_image.get_rect(topleft=(ecran_rect.right + 8, reset_button_rect.bottom + 20))

# Fonction pour gérer les clics sur les boutons R et S
def handle_simulation_buttons(mouse_pos):
    ...
    elif simulation_button_rect.collidepoint(mouse_pos):
...
reset_button_rect.topleft = (button_c_rect.left, button_c_rect.bottom + 60)
simulation_button_rect.topleft = (reset_button_rect.left, reset_button_rect.bottom + 10)

# Afficher les boutons R et S sous le bouton C
...
# Boucle principale
running = True
simulation_active = False  # Initialisation correcte
...
while running:
    ...
    # Dessiner les boutons R et S sous le bouton C
    screen.blit(reset_button_image, reset_button_rect.topleft)
    screen.blit(simulation_button_image, simulation_button_rect.topleft)

    # Ajouter les libellés sur les boutons R et S
